...
"#],
        ...
        ErrorExample {
            ...
            description: "`THIS` may only start a path expression (`THIS.member`, `THIS^.member`). It is not a member name, so it cannot appear after a dot. Using `THIS` in a POU that has no instance — a FUNCTION — is reported as E1105 instead.",
            sources: &[r#"
FUNCTION_BLOCK fb1
VAR
    x : INT;
...
"#],
        ...
        ErrorExample {
            ...
            description: "A method implementing an interface prototype or overriding a base method declares a different number of parameters. Parameters are matched by position; their names (E1128), sections (E1129) and types (E1126) and the return type (E1127) are checked one by one.",
            sources: &[r#"
CLASS Base
...
"#],
        ...
        ErrorExample {
            ...
            description: "A pragma that has no meaning on this POU kind: {once} on a PROGRAM. Placement of {test} and {extern} is a compiler error instead (E1503, E1501).",
            sources: &[r#"
{once}
PROGRAM main
END_PROGRAM
...
"#],
        ...
        ErrorExample {
            ...
            description: "A FUNCTION_BLOCK or PROGRAM call does not pass every declared `VAR_INPUT`. \
                          ...
                          input is the hard error E0802 instead.",
            sources: &[r#"
FUNCTION_BLOCK ramp
VAR_INPUT
    target : INT;
