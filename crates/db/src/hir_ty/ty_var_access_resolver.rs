...
use crate::{hir::{expressions::expression::{VariableAccess, VariableAccessKind}, scope::FileScopeId}, hir_ty::{ty::Ty, ty_path_expr_resolver::resolved_path_expr, TyResolved}};
...
impl<'db> VarAccessResolverCtx<'db> {
    pub fn new(db: &'db dyn crate::BaseDatabase, scope_id: FileScopeId, access: &'db VariableAccess<'db>) -> Self {
        Self { db, scope_id, access }
    }
