...
pub fn completion_descendant_at<'db>(
    ...
    // When best_match is None, the cursor is outside all nodes (e.g. after END_FUNCTION),
    // so we must NOT fall back to last_before — top-level completions should be shown.
