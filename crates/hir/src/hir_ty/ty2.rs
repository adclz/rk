...
use crate::{
    ...
    hir_def::{
        expressions::{
            expression::{
                BeginPathExpr, Elementary, Expr, ExprKind, FuncCall, InitExpr, Integer,
                ParamAssign, ParamAssignKind, PathExpr, PrimaryExpr, VariableAccess,
                VariableAccessKind,
            },
    ...
    hir_ty::{
        ...
        infer::ctx::InferCtx,
        inference::{Adjustment, InferenceError, InferenceResult},
    ...
    pub fn walk_begin_path_expr(
        ...
        if let Some(path) = expr.expr(db) {
            for step in path.flatten(db) {
                match current.walk_path_expr(db, step, ctx) {
                    Type::Never => break,
                    next => current = next,
                }
            }

            // return final type including adjustments
            return ctx
    ...
    pub fn walk_path_expr(
        ...
        match step {
            PathExprWalkStep::Deref {
                ...
                _ => {
                    ctx.errors.push(InferenceError::DerefNonRefType {
                        expr: *expr,
                        ty: *self,
                    });
                ...
                match self {
                    Type::Struct(st) => {
                        if let Some(field) = st.resolve_elements(db).get(&ident.ident) {
                            ...
                            ctx.errors.push(InferenceError::NoSuchField {
                                expr: *expr,
                                ident: **ident,
                                ty: *self,
                            });
                    ...
                    Type::Function(_) | Type::FunctionBlock(_) | Type::Class(_) => {
                        ...
                        // Other POUs
                        else if let Some(pou) = pou_names_res(db, ident.ident, ctx.scope) {
                            result_ty = Type::new_pou(db, pou);
                            ctx.type_of_path_expr.insert(*expr, result_ty);
                        } else {
                            ctx.errors.push(InferenceError::NoSuchField {
                                expr: *expr,
            ...
            PathExprWalkStep::Index { expr: _ } => match self {
                ...
                _ => {
                    ctx.errors.push(InferenceError::IndexNonArrayType {
                        expr: *expr,
                        ty: *self,
                    });
                    ctx.type_of_path_expr.insert(*expr, Type::Never);
