...
/// A part of a byte or more counts a FOR loop: it is whole bytes of its
/// owner's cell, with an address to count in. A bit could not, but a bit is
/// a BOOL, which is not a counter at all (E1206).
#[rstest]
fn valid_for_counter_in_a_part_of_a_byte_or_more(mut with_db: RootDatabase) {
    let source = r#"
PROGRAM P
