...
pub fn mir_exports(db: &mut RootDatabase, sources: &[&str]) -> String {
    ...
    for ext in &module.extern_functions {
        ...
        let ret = ext.return_type.as_ref()
            .map(|t| format!(" -> {}", fmt_ty(t, db)))
            .unwrap_or_default();
        let mono = ext.monomorphized_from
            .map(|m: Ident| format!(" [from {}]", m.text(db)))
            .unwrap_or_default();
        lines.push(format!("import {}.{}({}){}{}", ext.module, ext.import_name, params.join(", "), ret, mono));
    }
    ...
    for func in &module.functions {
        ...
        let ret = func.return_type.as_ref()
            .map(|t| format!(" -> {}", fmt_ty(t, db)))
            .unwrap_or_default();
        lines.push(format!("export {}({}){}", export_name, params.join(", "), ret));
    }
